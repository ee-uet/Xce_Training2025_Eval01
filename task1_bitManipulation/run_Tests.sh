...
# Test cases (input | expected output snippet check)
# num,k, m, n;(of exe file)
tests=(
...
for t in "${tests[@]}"; do
  ...
  expected=$(echo "$t" | cut -d'|' -f2- | xargs) # xargs trims whitespace


    # run program
    output=$($EXE <<< "$input") # here string to pass input
